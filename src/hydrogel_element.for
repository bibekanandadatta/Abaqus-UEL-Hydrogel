...
!     (1) hydrogel_general      : element formulation for 3D and 2D plane strain
!     (2) hydrogel_axisymmetric : element formulation for axisymmetry
...
! **********************************************************************
      SUBROUTINE hydrogel_general(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,
     ...
     &      .or. ((jtype .eq. 6) .and. (nInt .eq. 4)) ) then
          ...
          if (detJ0 .le. zero) then
            call msg%ferror( flag=warn, src='hydrogel_general',
     ...
     &          msg='Negative element jacobian: ', ivec=[jelem, intpt])
        ...
        do i=1,nNode
          ...
          ! form [Ba] matrix: plane stress/ plane strain case
            ...
            call msg%ferror( flag=error, src='hydrogel_general',
     ...
     &     Kelem,Relem)                                           ! output from the subroutine
      ...
      end subroutine hydrogel_general

! **********************************************************************
! **********************************************************************

      SUBROUTINE hydrogel_axisymmetric(RHS,AMATRX,SVARS,ENERGY,NDOFEL,
     & NRHS,NSVARS,PROPS,NPROPS,COORDS,MCRD,NNODE,Uall,DUall,Vel,Accn,
     & JTYPE,TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     & PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,NJPROPS,
     ...
     &                    nNode=nNode, nInt=nInt)
      ...
      if (fbarFlag .eq. .true.) then
        ...
        if ( (jtype .eq. 4) .and. (nInt .eq. 4) ) then
          ...
          if (detJ0 .le. zero) then
            call msg%ferror( flag=warn, src='hydrogel_axisymmetric',
     &      msg='Negative element jacobian at centroid: ', ia=jelem)
            pnewdt = fourth
            return
          end if
     ...
     &                        = Ga0(1:nDim**2+1,1:nDim)
          ...
          call msg%ferror( flag=error, src='hydrogel_axisymmetric',
     &      msg='F-bar is not available: ', ivec=[jtype, nInt])
          call xit
        end if
     ...
     &     Kelem,Relem)                                           ! output from the subroutine
      ...
      end subroutine hydrogel_axisymmetric
